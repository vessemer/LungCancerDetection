...
def operate(path):
    try:
        ...
        if mask.max() > 15:
            ...
            with open(join(PATH['STAGE_MASKS'], 'DATAFRAMES','no_enhs.txt'),'a') as c:
                c.write(path+'\n')
        ...
        iso_stats = collect_stats(isolated,mask,sp_mask)
        vas_stats = collect_stats(vascular,mask,sp_mask)
        plu_stats = collect_stats(plural,mask,sp_mask)
        with open(join(PATH['STAGE_MASKS'], 'DATAFRAMES', 'iso_stats.csv'), 'a') as fd:
                pd.DataFrame({'path': path, 
                              'stats': iso_stats}).to_csv(fd, header=False,index=False)
        with open(join(PATH['STAGE_MASKS'], 'DATAFRAMES', 'vas_stats.csv'), 'a') as fd:
                pd.DataFrame({'path': path, 
                              'stats': vas_stats}).to_csv(fd, header=False,index=False)
        with open(join(PATH['STAGE_MASKS'], 'DATAFRAMES', 'plu_stats.csv'), 'a') as fd:
                pd.DataFrame({'path': path, 
                              'stats': plu_stats}).to_csv(fd, header=False,index=False)        
        return (path, iso_stats),(path, vas_stats),(path, plu_stats)
    except:
        with open(join(PATH['STAGE_MASKS'], 'DATAFRAMES','crashes_feats.txt'),'a') as c:
            c.write(path+'\n')
        return (path, None),(path,None),(path,None)
...
patients = set([basename(path) for path in glob(join(PATH['STAGE_DATA'], '*'))])
#patients = patients.difference(set(pickle.load(open(join(PATH['WEIGHTS'], 'erroneus'), 'rb'))))
patients = list(patients)
#with open(join(PATH['STAGE_MASKS'],'DATAFRAMES','crashes_feats.txt')) as f:
#    patients = [pat.strip() for pat in f.readlines()]
#    patients = [pat for pat in patients if pat!='']
stats = list()
#with Pool(24) as p:
#    stats = p.map(operate,patients)

for pat in tqdm(patients):
    stats.append(operate(pat))

# In[ ]:

pickle.dump(stats, open(join(PATH['STAGE_MASKS'], 'DATAFRAMES', 'merged_stats'), 'wb'))
