...
def separate_lungs3d(file):
    interval = int(file.shape[1] * 0.1)
    start_slice_ind = -1
    start_slice = []
    with open('output_nrt','w') as f:
        print("finding start slice")
        for i in (range(file.shape[1] // 2 - interval, file.shape[1] // 2 + interval)):
            if (if_separate(file[:,i,:])):
                start_slice_ind = i
                break
        f.write(str(start_slice_ind))
        if (start_slice_ind == -1):
            start_slice = separate_lungs(file[:,file.shape[1] // 2,:], file.shape[1] // 2)
            start_slice_ind = file.shape[1] // 2
        else:
        ...
        for i in (range(start_slice_ind + 1, file.shape[1])):
